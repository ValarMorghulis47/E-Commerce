...
const getBarChartsData = TryCatch(async (_, res, __) => {
    ...
    let barCharts;
    barCharts = await redis.get("admin-bar-charts");
    if (barCharts) {
        barCharts = JSON.parse(barCharts);
    }
    else {
        const today = new Date();
        const sixMonthsAgo = new Date();
        ...
        await redis.setex("admin-bar-charts", redisTTL, JSON.stringify(barCharts));
    }
...
const getPieChartsData = TryCatch(async (_, res, __) => {
    ...
    let pieCharts;
    pieCharts = await redis.get("admin-pie-charts");
    if (pieCharts) {
        pieCharts = JSON.parse(pieCharts);
    }
    else {
        ...
        await redis.setex("admin-pie-charts", redisTTL, JSON.stringify(pieCharts));
    }
...
const getLineChartsData = TryCatch(async (_, res, __) => {
    ...
    let lineCharts;
    lineCharts = await redis.get("admin-line-charts");
    if (lineCharts) {
        lineCharts = JSON.parse(lineCharts);
    }
    else {
        const today = new Date();
        const tweleveMonthsAgo = new Date();
        ...
        await redis.setex("admin-line-charts", redisTTL, JSON.stringify(lineCharts));
    }
